...
# Import the function directly to avoid module import chain issues
def load_get_nm_elev():
    """Load get_nm_elev function directly from tools.py"""
    ...
    # Read and execute the tools.py file in a namespace
    namespace = {}
    with open(tools_path, "r") as f:
        # Import required modules for tools.py
        namespace["json"] = json
        namespace["logger"] = MagicMock()  # Mock logger to avoid import issues

        # Try importing url_request, if it fails, create a mock
        try:
            from urllib import request as url_request

            namespace["url_request"] = url_request
        except ImportError:
            namespace["url_request"] = MagicMock()

        exec(f.read(), namespace)

    return namespace["get_nm_elev"]


# Load the function once at module level
get_nm_elev = load_get_nm_elev()
