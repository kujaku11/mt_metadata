...
class TestTFCore(unittest.TestCase):
    ...
    def test_empty_equals(self):
        other_tf = TF()
        tfs_are_equal = self.tf.equals(
            other_tf, ignore_station_metadata_keys=["provenance.creation_time"]
        )
...
class TestTFEqual(unittest.TestCase):
    ...
    def test_full_tf_equals(self):
        self.assertTrue(
            self.tf_01.equals(
                self.tf_02, ignore_station_metadata_keys=["provenance.creation_time"]
            )
        )

    def test_full_tf_not_equals(self):
        self.tf_02.impedance = np.random.randn(3, 2, 2) + 1j * np.random.randn(3, 2, 2)
