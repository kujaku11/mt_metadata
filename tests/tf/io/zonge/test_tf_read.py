...
class TestEMTFXML(unittest.TestCase):
    ...
    def test_station_metadata(self):
        meta_dict = OrderedDict([('channels_recorded', ['ex', 'ey', 'hx', 'hy']),
                     ('data_type', 'nsamt'),
                     ('geographic_name', None),
                     ('id', '24'),
    ...
    def test_z(self):
        ...
        with self.subTest(msg="first element"):
            self.assertTrue(
                np.isclose(
                    self.tf.impedance[0],
                    np.array([[-0.85198341-1.1020768j , -1.76236149-1.89288924j],
        ...
        with self.subTest(msg="last element"):
            self.assertTrue(
                np.isclose(
                    self.tf.impedance[-1],
                    np.array([[  -1.90536913 -22.95806911j,  147.51100994+280.66686097j],
