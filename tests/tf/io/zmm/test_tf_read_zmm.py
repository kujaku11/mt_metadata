...
class TestEMTFXML(unittest.TestCase):
    ...
    def test_station_metadata(self):
        meta_dict = OrderedDict([('channels_recorded', ['ex', 'ey', 'hx', 'hy', 'hz']),
    ...
    def test_z(self):
        ...
        with self.subTest(msg="first element"):
            self.assertTrue(
                np.isclose(
                    self.tf.impedance[0],
                    np.array([[ -5.99100018 -5.95499992j,  17.27000046+12.72000027j],
        ...
        with self.subTest(msg="last element"):
            self.assertTrue(
                np.isclose(
                    self.tf.impedance[-1],
                    np.array([[  1.69399995e-04+0.01282j   ,   5.67400008e-02+0.06538j   ],
        ...
        with self.subTest(msg="first element"):
            self.assertTrue(
                np.isclose(
                    ...
                    np.array([[ 0.53822149,  1.44624196],
                           [ 1.92694986,  5.17786036]]),
                ).all()
            )
        ...
        with self.subTest(msg="last element"):
            self.assertTrue(
                np.isclose(
                    ...
                    np.array([[ 0.00273188,  0.0020596 ],
                           [ 1.12854018,  0.85081953]]),
                ).all()
            )
        ...
        with self.subTest(msg="first element"):
            self.assertTrue(
                np.isclose(
                    ...
                    np.array([[  18.05999947 -4.46999991e-07j,  -27.14999962 +6.88899994e+00j],
                           [ -27.14999962 +6.88899994e+00j,  130.39999390 +2.38400006e-07j]]),
                ).all()
            )
        ...
        with self.subTest(msg="last element"):
            self.assertTrue(
                np.isclose(
                    ...
                    np.array([[  1.92300007e-07 -4.44100010e-16j,
                              3.36799992e-08 -1.40400003e-08j],
                           [  3.36799992e-08 -1.40400003e-08j,
                              1.09299997e-07 +0.00000000e+00j]]),
                ).all()
            )
        ...
        with self.subTest(msg="first element"):
            self.assertTrue(
                np.isclose(
                    ...
                    np.array([[  1.60399992e-02+0.j       ,   2.29300000e-02+0.005487j ,
        ...
        with self.subTest(msg="last element"):
            self.assertTrue(
                np.isclose(
                    ...
                    np.array([[  3.88100014e+01 +0.00000000e+00j,
                             -1.31300000e+03 -2.84700000e+03j,
                              1.56499996e+01 -1.21499996e+01j],
                           [ -1.31300000e+03 +2.84700000e+03j,
                              6.62300000e+06 +0.00000000e+00j,
                              8.53800000e+03 +1.40900000e+04j],
                           [  1.56499996e+01 +1.21499996e+01j,
                              8.53800000e+03 -1.40900000e+04j,
                              1.17700000e+03 +0.00000000e+00j]]),
                ).all()
            )
