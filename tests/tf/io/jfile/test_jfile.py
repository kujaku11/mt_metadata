...
class TestJFile(unittest.TestCase):
    ...
    def test_title(self):
        self.assertEqual("BIRRP Version 5 basic mode output", 
                         self.j_obj.header.title)

    def test_birrp_parameters(self):
        birrp_params = OrderedDict([('ainlin', -999.0),
                     ('ainuin', 0.999),
                     ('c2threshe', 0.7),
                     ('c2threshe1', 0.0),
                     ('deltat', 0.1),
        ...
        self.assertDictEqual(birrp_params,
                             self.j_obj.header.birrp_parameters.to_dict(single=True))
        
    def test_data_blocks(self):
        db = [
            OrderedDict([('filnam', '/data/mtpy/examples/birrp_processing/birrp_wd/birrp_data_3.txt'), ('indices', 1), ('ncomp', 4), ('nread', 38750), ('nskip', 0)]), 
        OrderedDict([('filnam', '/data/mtpy/examples/birrp_processing/birrp_wd/birrp_data_3.txt'), ('indices', 3), ('ncomp', 4), ('nread', 38750), ('nskip', 0)])]
        
        for ii, block in enumerate(db):
            with self.subTest(msg=f"block {ii}"):
                self.assertDictEqual(block, self.j_obj.header.data_blocks[ii].to_dict(single=True))
