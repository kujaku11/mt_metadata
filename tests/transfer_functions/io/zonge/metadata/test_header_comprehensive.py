...
class TestHeaderCore:
    """Test core functionality of Header class."""
    ...
    @pytest.fixture(scope="class")
    def header_with_data(self):
        """Fixture providing Header with populated GPS data."""
        header = Header()
        header.name = "Test Station"
        header.elevation = 1500.0
        header.gps.lat = 45.123
        header.gps.lon = -123.456
        header.gps.datum = "WGS 84"  # Note: actual default has space
    ...
    def test_computed_properties(self, header_with_data, subtests):
        """Test computed properties work correctly."""
        with subtests.test("latitude property"):
            assert header_with_data.latitude == 45.123

        with subtests.test("longitude property"):
            assert header_with_data.longitude == -123.456

        with subtests.test("datum property"):
            assert header_with_data.datum == "WGS 84"  # Note: has space

        with subtests.test("station property"):
            assert header_with_data.station == "STATION_01"
    ...
    def test_serialization(self, header_with_data, subtests):
        """Test model serialization works."""
        with subtests.test("model_dump returns dict"):
            data = header_with_data.model_dump()
        ...
        with subtests.test("nested objects serialized"):
            data = header_with_data.model_dump()
            assert isinstance(data["gps"], dict)
            assert data["gps"]["lat"] == 45.123

        with subtests.test("model_dump_json works"):
            json_str = header_with_data.model_dump_json()
...
class TestHeaderProperties:
    """Test Header computed properties and custom setters."""
    ...
    def test_gps_properties(self, fresh_header, subtests):
        ...
        with subtests.test("datum property uppercase"):
    ...
    def test_properties_with_no_data(self, fresh_header, subtests):
        ...
        with subtests.test("datum is None when not set"):
            assert fresh_header.datum is None

        with subtests.test("station is None when not set"):
            assert fresh_header.station is None
...
class TestHeaderValidation:
    ...
    def test_elevation_validation(self, subtests):
        """Test elevation field validation."""
        with subtests.test("string elevation converted"):
            header = Header(elevation="1234.5")
            assert header.elevation == 1234.5

        with subtests.test("float elevation accepted"):
            header = Header(elevation=2000.0)
            assert header.elevation == 2000.0

        with subtests.test("None elevation defaults to 0"):
            header = Header(elevation=None)
...
class TestHeaderIntegration:
    ...
    def test_complete_workflow(self, subtests):
        ...
        with subtests.test("all data set correctly"):
            ...
            assert header.datum == "WGS84"
            assert header.station == "INT_TEST_01"

        # Test serialization
        data = header.model_dump()

        with subtests.test("serialization preserves data"):
            assert data["name"] == "Integration Test"
            assert data["elevation"] == 1800.0
            assert data["gps"]["lat"] == 46.5
            assert data["gps"]["lon"] == -121.7
    ...
    def test_model_operations(self, subtests):
        """Test Pydantic model operations."""
        header = Header(name="Model Test", elevation=1000.0)
        header.gps.lat = 45.0
        ...
        with subtests.test("model_validate works"):
            data = header.model_dump()
            new_header = Header.model_validate(data)
            assert new_header.name == "Model Test"
            assert new_header.elevation == 1000.0
