...
class TestMetronixEDI(unittest.TestCase):
    ...
    def test_header(self):
        head = OrderedDict([('acqby', 'UofAdel,Scripps,GA,GSSA,AuScope'),
    ...
    def test_info(self):
        info_list = ['SURVEY ID=Spencer Gulf',
                     'EASTING=683849', 
    ...
    def test_rhoxy(self):
        rho_xy = np.array([  0.2818635,   0.3512951,   0.3714305,   0.4108465,   0.5248638,
                 0.7726143,   1.108641 ,   1.672007 ,   2.658779 ,   3.556539 ,
                 4.978607 ,   6.450729 ,   8.247861 ,  10.09688  ,  42.33246  ,
               113.28     ,  62.97522  ,  31.13647  ,  38.18653  ,  34.51625  ,
                39.07644  ,  41.12259  ,  48.43526  ,  58.42656  ,  54.0161   ,
                72.11851  ,  72.32678  , 109.5934   ])
        
        rxy = (0.2 / self.edi_obj.frequency) * np.abs(self.edi_obj.z[:, 0, 1])**2
        self.assertTrue(np.isclose(rho_xy, rxy).all())
        
    def test_phsxy(self):
        phase_xy = np.array([35.75853 , 42.20026 , 36.66419 , 28.80162 , 21.99871 , 15.28629 ,
               11.75614 , 10.3068  ,  9.962963, 11.06905 , 12.87484 , 10.27247 ,
               14.81129 , 17.90185 , 12.38906 , 28.05279 , -3.029796, 26.9273  ,
               30.46892 , 35.27853 , 36.07981 , 36.34399 , 35.09986 , 34.32266 ,
               35.62811 , 37.02072 , 34.78629 , 33.30714 ])
        
        pxy = np.rad2deg(np.arctan2(self.edi_obj.z[:, 0, 1].imag, self.edi_obj.z[:, 0, 1].real))
        
        self.assertTrue(np.isclose(phase_xy, pxy).all())
            
    def test_rhoyx(self):
        rho_yx = np.array([2.581770e-01, 3.444989e-01, 3.612809e-01, 4.231358e-01,
               5.520689e-01, 8.359045e-01, 1.257848e+00, 2.410778e+00,
               3.774356e+00, 5.379387e+00, 1.055388e+01, 1.551731e+02,
               1.879535e+01, 4.052773e+01, 6.593614e+03, 6.227474e+04,
               2.134522e+04, 4.830593e+01, 4.689244e+01, 4.055480e+01,
               3.560237e+01, 3.621749e+01, 4.141123e+01, 4.527979e+01,
               4.039037e+01, 2.454437e+01, 2.248866e+01, 1.399194e+01])
        
        ryx = (0.2 / self.edi_obj.frequency) * np.abs(self.edi_obj.z[:, 1, 0])**2
        self.assertTrue(np.isclose(rho_yx, ryx).all())
        
    def test_phsyx(self):
        phase_yx = np.array([ 36.69456,  42.27999,  35.73111,  27.35834,  20.00085,  13.55288,
                10.96114,  15.85582,  19.43036,  20.81004,  18.32144,  57.72337,
                51.8989 ,  22.68562, -61.66165, -21.01045, -36.26306,  35.76523,
                42.78572,  47.3067 ,  46.58979,  45.80391,  47.74424,  50.75497,
